// Written by Joshua Wry (100661785)
...
public class Game_Content {
	...
	// Initialize the Width and height of the window, along with all images to be used.
	...
	// board is where everything in the game happens.
	...
	// 4 is default Normal difficulty.
	...
	// Initializing variables, added it to the group.
	private void setUp() {
		...
		hero = new ImageView(new Image(PLAYER_IMAGE));
	...
	// enemyCounter is the number of enemies currently on the screen. 
	int enemyCounter = 0; 
	
	// AnimationTimer is a functional interface, and allows
	// the handle function to run 30 times a second. 
	...
	// Creates an Enemy (Tie Fighters)
	public void createEnemy() {
		//Enemy enemy = new Enemy();
		ImageView enemy = new ImageView(ENEMY_IMAGE);
		Node newEnemy = enemy; 
	...
	// Moves each enemy
	// Note: Each enemy has a different movement range
	// Enemies 1, 2, and 3 actively track the player.
	// Enemies 4 and 5 move around randomly in their ranges.
	...
	// Movement variables specific to enemy 4. 
	int fodx = -2;
	int fody = 2; 
	...
	//Movement variables specific to enemy 5. 
	int fidx = -2; 
	int fidy = 2;
	...
	// Creates the projectiles that the enemies shoot.
	// Each enemy has an array that keeps track of their
	// on screen missiles. 
	public void createEnemyMissile() {
		
		// Each enemy needs their own unique missiles
	...
	// Moves each enemy's missile by looping through their 
	// lists of on-screen missiles and adjusting their positions. 
	...
	// There are two background images, together 
	// that the window is always showing the right background
	// by moving into the screen whenever the other is moving out
	// of it. 
	...
	// Determines when enemies fire their missiles. 
	...
	// Stops moving when the key is released.  
	...
	// This is what occurs when you collect the power up.
	// Missiles rain down across the screen, hitting everything
	// but the player. 
	...
	// Fires the missiles from the bombardment.
	...
	// This method checks for intended intersections
	// between two images. 
	// Enemy missiles can only hit the player. 
	// Player missiles can hit asteroids and enemies. 
	// The player can crash into asteroids, or power ups. 
	// Bombardment effects enemies and asteroids. 
